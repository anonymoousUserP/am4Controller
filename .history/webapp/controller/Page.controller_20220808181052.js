sap.ui.define(
    ...
    function (mobileLibrary,coreLibrary,MessageToast, PieChart, BaseController, JSONModel, ODataModel, Fragment,ColorPickerPopover,unified) {
        "use strict";
        ...
        return BaseController.extend("sap.m.sample.Menu.controller.Page", {
            onSave: function () {
                ...
                mobileLibrary.URLHelper.redirect(url);
            ...
            downloadPress : function(){
                var data = this.getView().getModel('configureModel').getData();
            ...
            editDataPress : function(){
                ...
                let val = this.getView().getModel('configureModel').getData().value;
            ...
            editDataSavePress : function(){
                ...
                const oData = this.getView().getModel('oModel').getData().selCatValList;
                let resultData = this.getView().getModel('configureModel').getData().chartData;
                const cat = this.getView().getModel('configureModel').getData().category;
                const val = this.getView().getModel('configureModel').getData().value;

                for(let i=0;i<oData.length;i++){
                    resultData[i][cat] = oData[i]['Cat'];
                    resultData[i][val] = Number(oData[i]['Val']);
                }
                debugger;
                this.getView().getModel('configureModel').getData().chartData = resultData;
